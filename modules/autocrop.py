...
class CropImageToMaskBounds:
    """
    Crop an IMAGE using a MASK, then optionally center the crop on a
    ...
    @staticmethod
    def _pad_image_and_mask_to_square(
        image: torch.Tensor,
        mask: torch.Tensor,
        square_margin: int,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Center one cropped image and mask on a square canvas.
    ...
    def crop(
        ...
        for index in range(output_count):
            ...
            # The image and mask should normally already match.
            if (
            ...
            if square_pad:
                cropped_image, cropped_mask = (
                    self._pad_image_and_mask_to_square(
                        cropped_image,
                        cropped_mask,
...
NODE_CLASS_MAPPINGS = {
    "CropImageToMaskBounds": CropImageToMaskBounds,
}


NODE_DISPLAY_NAME_MAPPINGS = {
    "CropImageToMaskBounds": "Crop Image to Mask Bounds and Square",
}